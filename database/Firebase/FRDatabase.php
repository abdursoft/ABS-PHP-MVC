...
use DB\Firebase\Firebase;
use PDO;

class FRDatabase extends Firebase
{
    // Email Validation 
    ...
    {
        try {
            self::$db->getReference("$table/2")->set($input);
            return true;
        } catch (\Throwable $th) {
            return false;
        }
    ...
    public static function getLastID($table,$name)
    {
        return self::$db->getReference($table)->orderByChild($name)->limitToLast(1)->getSnapshot();
    }
    ...
    public static function dataSelectObject($table, $key, $option=null)
    {
       $result = self::$db->getReference($table)->getSnapshot();
